...
// ─── Module-level mirror state ────────────────────────────────────────────────
// Set once when the child profile is fetched; read by launchMirrorPortal.
// Never exposed globally — parents cannot reach another child's data.
...
window.loadConsoleData = async (profile) => {
    ...
    // Store on window so launchMirrorPortal can read linked_children as fallback
...
// ─────────────────────────────────────────────────────────────────────────────
// INTERCOM LISTENER
// Uses the child's actual grade/section (populated after fetchChildData).
// Falls back to "9"/"A" only if _mirrorChildClassId is not yet resolved.
// ─────────────────────────────────────────────────────────────────────────────
async function listenToIntercom() {
    ...
    // Use the resolved child grade/section; do NOT fall back to a hardcoded "9"
    let targetGrade   = _mirrorChildClassId || null;
    let targetSection = _mirrorChildSection || null;

    // If module vars aren't filled yet (race condition), try to fetch from
    // linked_children directly so we never default to the wrong grade.
    if (!targetGrade && profile.linked_children?.length) {
        try {
            const linkedSnap = await getDoc(doc(db, "users", profile.linked_children[0]));
            if (linkedSnap.exists()) {
                const d = linkedSnap.data();
                targetGrade   = d.classId   || null;
                targetSection = d.section   || null;
            }
        } catch (e) {
            console.warn("Could not resolve child grade for intercom.", e);
        }
    }
    ...
    onSnapshot(q, (snapshot) => {
        ...
        docs.forEach(d => {
            const data = d.data();
            // Only show messages targeting THIS child's grade & section
            if (targetGrade && data.target_grade !== targetGrade) return;
            if (targetSection && data.target_section !== targetSection) return;
...
// ─────────────────────────────────────────────────────────────────────────────
// FETCH CHILD DATA
// Populates module-level mirror vars as soon as the child profile is loaded.
// ─────────────────────────────────────────────────────────────────────────────
async function fetchChildData(parentProfile) {
    try {
        ...
        if (parentProfile.linked_children && parentProfile.linked_children.length > 0) {
            ...
            // ── Resolve child profile and populate mirror vars ────────────────
            try {
                ...
                if (childSnap.exists()) {
                    ...
                    _mirrorChildClassId = childData.classId  || null;
                    _mirrorChildSection = childData.section  || null;
                }
            } catch (e) {
                console.warn("Could not fetch child profile:", e);
                // Still set the UID so the mirror button at least works
                _mirrorChildUid = targetUid;
            }
        }
        ...
        window._switchChild = async (uid) => {
            ...
            if (snap?.exists()) {
                const d = snap.data();
                childName           = d.displayName || "Student";
                _mirrorChildUid     = uid;
                _mirrorChildClassId = d.classId  || null;
                _mirrorChildSection = d.section  || null;
            } else {
                _mirrorChildUid = uid;
            }
            const contextBadge = document.getElementById("context-badge");
            if (contextBadge) contextBadge.innerHTML = `<i class="fas fa-child mr-1"></i> ${childName}`;
            await renderSyncWallAndInbox(db, uid, {});
        };

        if (!targetUid) {
            console.error("No linked student found for parent.");
            const matrixBody = document.getElementById("matrix-body");
            if (matrixBody) matrixBody.innerHTML = `<tr><td colspan="5" class="py-4 text-center text-slate-500 text-xs">No linked student found. Please contact admin.</td></tr>`;
            const syncWall = document.getElementById("sync-wall-container");
            if (syncWall) syncWall.innerHTML = `<div class="text-sm text-slate-500 italic p-4 text-center">No student linked to this parent account.</div>`;
            return 0;
        }
        ...
        try {
            ...
            console.error("Critical Permission Error fetching quiz_scores.", e);
            const matrixBody = document.getElementById("matrix-body");
            if (matrixBody) matrixBody.innerHTML = `<tr><td colspan="5" class="py-4 text-center text-slate-500 text-xs">Permission Denied: Unable to fetch actual student data.</td></tr>`;
            const syncWall = document.getElementById("sync-wall-container");
            if (syncWall) syncWall.innerHTML = `<div class="text-sm text-slate-500 italic p-4 text-center">Data access restricted.</div>`;
            return 0;
        }
        ...
        if (snapshot.empty) {
            ...
            const matrixBody = document.getElementById("matrix-body");
            if (matrixBody) matrixBody.innerHTML = `<tr><td colspan="5" class="py-4 text-center text-slate-500 text-xs">No quiz data available to generate Report Card.</td></tr>`;
        ...
        const totalScore = mcqScore + arScore + caseScore;
        const totalQs    = totalMCQ + totalAR + totalCase;
        const overallAvg = totalQs > 0 ? Math.round(totalScore / totalQs) : 0;
...
// ─────────────────────────────────────────────────────────────────────────────
// LAUNCH MIRROR PORTAL
//
// Security model:
//   • childUid comes from the parent's OWN Firestore profile (linked_children),
//     which is guarded by Firestore rules — a parent can never read another
//     user's document to obtain a foreign UID.
//   • sessionStorage is scoped to the current browser tab/origin; no cross-user
//     leakage.
//   • The student console must validate that the uid in sessionStorage passes
//     the isParentOfStudent() rule before rendering any data.
...
window.launchMirrorPortal = async function () {
    ...
    // Fallback: re-derive from window.userProfile if module vars are somehow empty
    if (!childUid) {
        const profile = window.userProfile;
        if (!profile?.linked_children?.length) {
            alert("No linked student found. Please contact your school administrator.");
            return;
        }
        ...
        try {
            ...
            console.warn("Mirror portal: could not resolve child grade.", e);
        }
    }
...
async function renderSyncWallAndInbox(db, childUid, chapterData) {
    ...
    try {
        ...
        if (!notifDocs.empty) {
            notifDocs.forEach(docSnap => {
                ...
                const statusLabel = hasScore ? 'Completed' : (hoursDiff <= 48 ? 'Pending' : 'Escalated');
                ...
                <div class="bg-white border border-slate-100 rounded-2xl p-3 hover:border-blue-100 transition shadow-sm">
                    <div class="flex items-start justify-between gap-3">
                        <div class="min-w-0">
                            <div class="text-[10px] uppercase tracking-widest font-black ${statusColor}">${statusLabel}</div>
                            <div class="font-bold text-slate-800 text-xs mt-1 break-words">
                                ${(data.chapter_title || chap).replaceAll("_", " ").replaceAll("quiz", "")}
                            </div>
                        </div>
                    ...
                    <div class="bg-white border border-slate-100 rounded-2xl p-4 hover:border-blue-100 transition shadow-sm">
                        <div class="flex items-start justify-between gap-3">
                            <div class="flex items-start gap-3 min-w-0">
                                ...
                                <div class="min-w-0">
                                    <div class="text-[10px] uppercase tracking-widest font-black text-cbse-blue mb-1">Teacher Trigger</div>
                                    <div class="font-bold text-slate-800 text-sm truncate">${chap.replaceAll("_", " ")}</div>
                                    <div class="text-xs text-slate-500 mt-1">Autonomous execution window active.</div>
                                </div>
                            </div>
                            <div class="text-[10px] font-black text-cbse-blue whitespace-nowrap">${hoursDiff}h</div>
                        </div>
                    </div>`;
                    ...
                    <div class="bg-white border border-amber-100 rounded-2xl p-4 hover:border-warning-yellow transition shadow-sm">
                        <div class="flex items-start justify-between gap-3">
                            <div class="flex items-start gap-3 min-w-0">
                                ...
                                <div class="min-w-0">
                                    <div class="text-[10px] uppercase tracking-widest font-black text-warning-yellow mb-1">Escalation Protocol</div>
                                    <div class="font-bold text-slate-800 text-sm truncate">${chap.replaceAll("_", " ")}</div>
                                    <div class="text-xs text-slate-500 mt-1">Pending beyond execution window.</div>
                                </div>
                            </div>
                            <div class="text-[10px] font-black text-danger-red whitespace-nowrap">${hoursDiff}h</div>
                        </div>
                    </div>`;
                }
            });
    ...
    // Fallback diagnostics for chapters with scores but no notification record
    for (const [chap, scoreData] of Object.entries(chapterData)) {
        ...
        if (latestScore < 95) {
            ...
            <div class="bg-white border border-red-100 rounded-2xl p-3 hover:border-danger-red transition shadow-sm">
                <div class="flex items-start justify-between gap-3">
                    <div class="min-w-0">
                        <div class="text-[10px] uppercase tracking-widest font-black text-danger-red">Re-Attempt Required</div>
                        <div class="font-bold text-slate-800 text-xs mt-1 break-words">${chap.replaceAll("_", " ")}</div>
                        <div class="text-[10px] text-slate-400 mt-1">Mastery below 95%</div>
                    </div>
                    <div class="text-[10px] font-black text-danger-red whitespace-nowrap">${latestScore}%</div>
                </div>
            </div>`;
        } else {
            syncHtml += `
            <div class="bg-white border border-green-100 rounded-2xl p-4 opacity-80 hover:opacity-100 transition shadow-sm">
                <div class="flex items-start justify-between gap-3">
                    <div class="flex items-start gap-3 min-w-0">
                        ...
                        <div class="min-w-0">
                            <div class="text-[10px] uppercase tracking-widest font-black text-success-green mb-1">Mastered</div>
                            <div class="font-bold text-slate-800 text-sm truncate">${chap.replaceAll("_", " ")}</div>
                        </div>
                    </div>
                    <div class="text-[10px] font-black text-success-green whitespace-nowrap">${latestScore}%</div>
                </div>
            </div>`;
        }
    }
    ...
    if (priorityCount > 0) {
        ...
        inboxList.innerHTML = `<div class="space-y-3 max-h-[420px] overflow-y-auto pr-1">${inboxHtml}</div>`;
    } else {
        if (inboxBadge) inboxBadge.classList.add("hidden");
        inboxList.innerHTML = `<div class="text-xs text-slate-400 text-center py-4">No priority alerts.</div>`;
    }
}
...
function renderMatrix(chapterData) {
    ...
    for (const [chap, data] of Object.entries(chapterData)) {
        ...
        grouped[subject].push({ chap, ...data });
    }
    ...
    Object.entries(grouped).forEach(([subject, chapters]) => {
        ...
        if (avg >= 95)      { strength = "MASTERED";  strengthColor = "text-success-green bg-green-50 border-green-100"; }
        else if (avg >= 80) { strength = "STRONG";    strengthColor = "text-cbse-blue bg-blue-50 border-blue-100"; }
        else if (avg >= 60) { strength = "MODERATE";  strengthColor = "text-warning-yellow bg-amber-50 border-amber-100"; }
        ...
        <div class="bg-slate-50 border border-slate-100 rounded-2xl overflow-hidden">
            <button onclick="toggleSubjectCard(this)"
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div class="text-left">
                        <div class="font-black text-slate-900 text-sm uppercase tracking-wide">${subject}</div>
                        <div class="text-[11px] text-slate-400 font-semibold mt-0.5">${chapters.length} Chapters • ${avg}% Avg Mastery</div>
                    </div>
                </div>
                <div class="flex items-center gap-3">
                    <div class="px-3 py-1 rounded-full border text-[10px] font-black ${strengthColor}">${strength}</div>
        ...
        chapters.forEach(data => {
            const latest = Math.max(data.simple || 0, data.medium || 0, data.advanced || 0);
            ...
            <div class="grid grid-cols-12 gap-2 px-5 py-4 border-b border-slate-50 hover:bg-slate-50 transition items-center">
                <div class="col-span-5 min-w-0">
                    <div class="font-bold text-slate-800 text-sm truncate">${data.chap}</div>
                    <div class="text-[10px] text-slate-400 mt-1">${data.attempts} attempts • Last ${data.lastDate || 'N/A'}</div>
                </div>
                <div class="col-span-2 text-center">${getBadgeHtml(data.simple)}</div>
                <div class="col-span-2 text-center">${getBadgeHtml(data.medium)}</div>
                <div class="col-span-2 text-center">${getBadgeHtml(data.advanced)}</div>
...
function renderAnalyzer(mcqCount, mcqSum, arCount, arSum, caseCount, caseSum) {
    ...
    if (mcqEl)  { mcqEl.style.width  = `${mcqPct}%`;  document.getElementById("txt-mcq").textContent  = `${mcqPct}%`;  mcqEl.className  = `h-full rounded-full ${getBarColor(mcqPct)}`;  }
    if (arEl)   { arEl.style.width   = `${arPct}%`;   document.getElementById("txt-ar").textContent   = `${arPct}%`;   arEl.className   = `h-full rounded-full ${getBarColor(arPct)}`;   }
    if (caseEl) { caseEl.style.width = `${casePct}%`; document.getElementById("txt-case").textContent = `${casePct}%`; caseEl.className = `h-full rounded-full ${getBarColor(casePct)}`; }
}
