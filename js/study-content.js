...
// Dynamic import of the curriculum based on URL grade (Board classes focus)
let engine;
async function initEngine() {
    try {
        ...
        console.error("Failed to load grade curriculum. Defaulting to 10.");
    }
}
await initEngine();
...
const observer = new MutationObserver(() => {
    ...
    if (originalTestBtn && !originalTestBtn.dataset.moved) {
        hijackTestButton(originalTestBtn);
    }
});
observer.observe(document.getElementById('content-container'), { childList: true, subtree: true });

function hijackTestButton(oldBtn) {
    oldBtn.dataset.moved = "true";
    oldBtn.classList.add('hidden'); // Hide original renderer button
    ...
    newBtn.onclick = () => {
        openDifficultyModal({ 
            ...
            chapter: params.get('chapter') || 'Real_Numbers' 
        });
    };
...
document.getElementById('trigger-board-insights').addEventListener('click', async () => {
    ...
    const chapterKey = chapter.toLowerCase().replace(/ /g, '_');
    ...
    if (!data || !data.content.insights?.length) {
        ...
        try {
            ...
            alert("A connection is required for the first sync.");
            return;
        }
    }
    ...
    target.innerHTML = `
        <div class="bg-white border-4 border-cbse-blue rounded-[2.5rem] overflow-hidden shadow-2xl mb-12">
            <div class="bg-cbse-blue p-6 text-white flex justify-between items-center">
                <div>
                    <h2 class="text-xl font-bold">${data.chapter_name} Board Pattern</h2>
                    <p class="text-blue-100 text-xs italic">Authentic Extractions</p>
                </div>
                    ...
                    <div class="border-l-4 border-accent-gold pl-6 md:pl-10">
                        ...
                        ${insight.image_path ? `
                            <div class="mb-10 rounded-2xl overflow-hidden shadow-md border border-slate-100 bg-slate-50">
                                <div class="bg-slate-100 py-2 text-[9px] font-black text-slate-400 uppercase tracking-widest text-center">Reference Visual from PDF</div>
                                ...
                                <div>
                                    ...
                                    <details class="mt-5 ml-4 group">
                                        <summary class="text-[10px] font-black text-cbse-blue cursor-pointer list-none uppercase tracking-[0.2em] hover:text-accent-gold transition">
                                            <i class="fas fa-key mr-1"></i> View Official Answer
                                        </summary>
...
async function renderGuidance() {
    ...
    try {
        ...
        else if (bestSimple < 85) msgEl.innerHTML = `Current Focus: Master Simple Level (85%+)`;
        else msgEl.innerHTML = `Great Work! Move to next Proficiency`;
    } catch (e) {}
}
...
function openDifficultyModal(ctx) {
    ...
    modal.className = `fixed inset-0 bg-slate-900/90 backdrop-blur-md z-50 flex items-center justify-center p-4`;
    modal.innerHTML = `
        <div class="bg-white rounded-[2.5rem] w-full max-w-md p-10 shadow-2xl relative border-b-8 border-cbse-blue text-center">
...
function createCard(level, sub) {
    return `<button onclick="launchQuiz('${level}')" class="w-full p-5 bg-slate-50 border-2 border-slate-100 rounded-3xl hover:bg-cbse-blue hover:text-white transition-all text-left flex items-center justify-between">
        <div class="font-black text-xl">${level}</div><div class="text-[10px] font-black uppercase opacity-60">${sub}</div>
    </button>`;
}
