...
function openDifficultyModal(ctx) {
    ...
    modal.className = `fixed inset-0 bg-slate-900/95 backdrop-blur-xl z-50 flex items-center justify-center p-4 fade-in`;
    modal.innerHTML = `
        <div class="bg-white rounded-[3rem] w-full max-w-md p-12 shadow-2xl relative border-b-8 border-cbse-blue text-center">
            <button onclick="this.closest('.fixed').remove()" class="absolute top-8 right-8 w-12 h-12 flex items-center justify-center bg-slate-100 rounded-full text-slate-400 hover:text-red-500 transition">✕</button>
            <h3 class="text-2xl font-black text-slate-900 mb-2">Target Proficiency</h3>
            <p class="text-[10px] font-black text-cbse-blue uppercase tracking-[0.3em] mb-12">${ctx.subject}</p>
            <div class="space-y-4">
                ${['Simple', 'Medium', 'Advanced'].map(lvl => `
                    <button onclick="launchQuiz('${lvl}')" class="w-full p-6 bg-slate-50 border-2 border-slate-100 rounded-[2rem] hover:bg-cbse-blue hover:text-white transition-all text-left flex items-center justify-between font-black text-xl">
                        ${lvl} <i class="fas fa-chevron-right opacity-30 text-sm"></i>
                    </button>
                `).join('')}
            </div>
        </div>`;
